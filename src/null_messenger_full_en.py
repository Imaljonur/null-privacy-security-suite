...
# -*- coding: utf-8 -*-
"""
null_messenger_pfs_full.py — Tor Messenger (Server/Client), none History, grau/grün UI
Now with: X25519-ECDH (PFS) pro Verbindung + Session-Key, Key-Rotation, File-Transfer über Session-Key,
Rooms, Admin, Outbox. Gray/Green UI.
"""
...
def _start_tor(torrc_path: Path) -> subprocess.Popen:
    if not TOR_BIN.exists(): raise FileNotFoundError(f"Tor nicht gefunden: {TOR_BIN}")
...
def _wait_for_onions() -> Dict[str,str]:
    ...
    while time.time()-t0 < 120:
        ...
        time.sleep(0.2)
    raise TimeoutError("Hidden Service hostname wurde nicht erstellt (Timeout).")
...
class NullMessenger(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(self.sidebar, text="Modus", font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", padx=10, pady=(10,4))
        ...
        ctk.CTkLabel(self.sidebar, text="Onion (client target / host below)", text_color=THEME["muted"]).pack(anchor="w", padx=10, pady=(12,4))
        ...
        ctk.CTkButton(nc_row, text="🔐 NC-Encrypt (Input)", command=self._nc_encrypt_entry).grid(row=0, column=2, padx=4)
        ctk.CTkButton(nc_row, text="🔓 NC-Decrypt (Input)", command=self._nc_decrypt_entry).grid(row=0, column=3, padx=4)
    ...
    def role_dialog(self):
        dlg = ctk.CTkInputDialog(text="Select mode: server or client", title="Startup mode")
    ...
    def start_action(self):
        ...
        if not self.password:
            messagebox.showinfo("Note", "Bitte Passwort setzen (for E2E-Krypto)."); return
    ...
    def start_server(self):
        ...
        try:
            ...
            messagebox.showerror("Hidden service error", str(e)); self.set_status("Hidden service error.")

    def _handle_chat_client(self, sock: socket.socket):
        addr = sock.getpeername(); self.log(f"[+] Chat connection from {addr}")
        ...
        try:
            ...
            if room != self.room:
                self.log(f"[x] Client in anderem Room: {room} (Server-Room: {self.room}) — disconnect."); sock.close(); return
            # Bans
            ...
            self.set_connected_state(True); self.header.configure(text=f"Connected to {nick}")
            ...
            if self.outbox:
                ...
                if sess:
                    ...
                    self.outbox.clear(); self.set_status("Delivered: outbox cleared")
        except Exception:
            try: sock.close()
            except Exception: pass
            return
            ...
            while True:
                ...
                if t == "msg":
                    ...
                    try:
                        ...
                        text = "[Decrypt-Fehler: {}]".format(e)
                    nick_in = self.clients_map.get(sock) or obj.get("nick") or "Peer"
                    self.log(f"{nick_in}: {text}")
                elif t == "file":

                    # File-Stream über Chat-Socket (verschlüsselt per Session-Key)

                    sess = self.sessions.get(sock)

                    if not sess: self.log("[x] File ohne Session-Key ignoriert."); continue
                    ...
                    outdir = filedialog.askdirectory(title="Zielordner for File speichern")
                    ...
                    try:
                        ...
                        if self._replay.add_or_dup(mid):
                            ...
                            self.log("[i] Replay File gedroppt.");
                        ...
                        self.log(f"[✓] File gespeichert: {out_path}")

                    except Exception as e:

                        self.log("[x] File-Decrypt fehlgeschlagen: {}".format(e))
    ...
    def _handle_file_client(self, sock: socket.socket):
        # Nicht mehr genutzt: Fileübertragung läuft über Chat-Socket mit Session-Key
        try: sock.close()
        except Exception: pass
    ...
    def start_client(self):
        ...
        self.set_status("Starte Tor (Client)…"); self.set_connected_state(False)
        ...
        if not _wait_port_open("127.0.0.1", int(self.socks_port), timeout=60):
            messagebox.showerror("Tor error", f"SOCKS port {self.socks_port} wurde nicht bereit in 60s."); return
        ...
        while time.time()-start_t < 60:
            try:
                ...
                self.header.configure(text=f"Connected to {onion}")
                self.set_status(f"Client via SOCKS {self.socks_port}")
                ...
                # Start Receiver-Thread for Rekey-Acks (und evtl. Server-Nachrichten in Zukunft)
                threading.Thread(target=self._client_recv_loop, daemon=True).start()
                return
            except Exception as e:
                last_err=e; time.sleep(1.0)
        messagebox.showerror("Connection failed", f"Could not connect within 60s: {last_err}")
    ...
    def send_msg(self):
        ...
        if text.lower().startswith("/color "):
            ...
            if not (len(val) in (4,7) and val.startswith("#") and all(c in "0123456789abcdefABCDEF#" for c in val)):
                self.set_status("Invalid color. Use e.g. #666666 oder #6a6")
                self.entry.delete(0,"end"); return
            self.my_color = val
        ...
        if self.role == "server" and text.startswith("/"):
            ...
            if cmd == "/fingerprint":
                ...
                self.entry.delete(0,"end"); return
            if cmd == "/users":
                try: names=[self.clients_map.get(s,"Peer") for s in self.clients]; self.set_status("Users: " + (", ".join(names) if names else "No users"))
        ...
        try:
            if self.role == "server":
                ...
                if not self.active_sock or not self.session_password:
                    messagebox.showinfo("Note", "No session key (handshake not finished)."); return
                # Rotation falls nötig
                ...
                mid = base64.b64encode(os.urandom(12)).decode("ascii")
                if not self._rate.allow():
                    messagebox.showwarning("Rate limit","Too many messages — please wait."); return
                cnt = getattr(self, "_client_count", 0)
                pad = _rand_pad_b64()
                payload = json.dumps({"t": text, "pad": pad})
    ...
    def send_file(self):
        ...
        try:
            ...
            if self.role == "server":
                ...
                if not self.active_sock or not self.session_password: messagebox.showinfo("Note","No session key (handshake not finished)."); return
                if not self._rate.allow():
                    return
            ...
            self.log(f"📦 File gesendet: {p.name} ({p.stat().st_size} Bytes)")
        except Exception as e:
            messagebox.showerror("Send fehlgeschlagen", str(e))
    ...
    # --- NullCrypto manuell: Fileen ---
    def _nc_encrypt_file_dialog(self):
        try:
            pw = (self.nc_pw.get().strip() if hasattr(self, 'nc_pw') else '')
            if not pw:
                self.set_status('NullCrypto: password missing'); return
            path = filedialog.askopenfilename(title='File wählen zum NC-Verschlüsseln')
            if not path: return
            out = encrypt_file(path, None, pw, aad_text=self.room, kdf_profile=self.kdf_profile)
            self.log(f'[NC] Encrypted → {out}')
            self.set_status('NullCrypto: File verschlüsselt')
        except Exception as e:
    ...
    def _nc_decrypt_file_dialog(self):
        try:
            ...
            if not pw:
                self.set_status('NullCrypto: password missing'); return
            path = filedialog.askopenfilename(title='NC-verschlüsselte File wählen (.scube)')
            if not path: return
            out = decrypt_file(path, None, pw, aad_text=self.room, kdf_profile=self.kdf_profile)
            self.log(f'[NC] Decrypted → {out}')
            self.set_status('NullCrypto: File entschlüsselt')
        except Exception as e:
